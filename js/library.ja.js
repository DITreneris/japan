(function() {
            ...
            function copyPrompt(button, promptId) {
                ...
                if (!button || !promptId) {
                    showError(button, '問題が発生しました。もう一度コピーしてください。');
                    return;
                }
            ...
            function fallbackCopy(text, button) {
                ...
                if (!textarea) {
                    showError(button, '問題が発生しました。もう一度お試しください。テキストを選択して Ctrl+C（または Cmd+C）でコピーできます。');
                    isCopying = false;
                    return;
                }
            ...
            function showSuccess(button) {
                ...
                setTimeout(() => {
                    ...
                    if (promptId) {
                        button.setAttribute('aria-label', `プロンプト${promptId.replace('prompt', '')}をクリップボードにコピー`);
                    }
            ...
            function showError(button, message) {
                ...
                const original = button.innerHTML;
                const errorMessage = message || '問題が発生しました。もう一度コピーしてください。';
